...
class Attention(nn.Module):
    ...
    def forward(self, x):
        B, N, C = x.shape
        # permute将tensor的维度换位:torch.Size([2, 3, 5]).permute(2, 0, 1).size() --->  torch.Size([5, 2, 3])
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        # flatten:扁平化,假设你的数据为1维数据，那么这个数据天然就已经扁平化了，如果是2维数据，那么扁平化就是将2维数据变为1维数据
        x = self.proj(x).flatten(2).transpose(1, 2)
        return x



# a = PatchEmbed(in_chans=2)
# b = torch.rand(5, 2, 224, 224)
# print(a(b).shape)
# # torch.rand(5, 3, 224, 224) --> torch.Size([5, 196, 768])
...
class VisionTransformer(nn.Module):
    ...
    def get_selfattention_and_feature(self, x):
        attn_weights = []
        x = self.prepare_tokens(x)
        for i, blk in enumerate(self.blocks):
    ...
    def get_all_layers_feature(self, x):
        x = self.prepare_tokens(x)
        # we return the all layers feature tokens
        output = []
        for i, blk in enumerate(self.blocks):
            x = blk(x)
            output.append(x)
