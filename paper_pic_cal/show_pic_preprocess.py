...
from simCLR.pic_pre_process import get_random_RB_all, pic_rotate_by_Rb, get_random_RB_curve_2, pic_add_noise, \
    pic_tailor, pic_mirror
...
data_depth = a.depth_data[200: 500, :]
RB_index = np.random.randint(0, 2)
dyna_org = a.img_dyna[200: 500, :]
stat_org = a.img_static[200: 500, :]
stat_org = stat_org * 0.1
...
pic_dyna_enhance = pic_enhence_random(pic_dyna, windows_shape=3, ratio_top=0.2, ratio_migration=0.7, random_times=3)
pic_dyna_noised = pic_add_noise(pic_dyna, ratio=0.1)
...
pic_shape = (224, 224)
pic_dyna_enhance_tailor = pic_tailor(pic_dyna_enhance, pic_shape)
pic_stat_enhance_tailor = pic_tailor(pic_stat, pic_shape)
print(pic_dyna_enhance_tailor.shape, pic_stat_enhance_tailor.shape)

# pic_dyna_enhance_tailor_mirror = pic_mirror(pic_dyna_enhance_tailor)

pic_dyna_enhance_tailor_reshape = cv2.resize(pic_dyna_enhance_tailor, pic_shape)
pic_stat_enhance_tailor_reshape = cv2.resize(pic_stat_enhance_tailor, pic_shape)
print(pic_dyna_enhance_tailor_reshape.shape, pic_stat_enhance_tailor_reshape.shape)

show_Pic([dyna_org, stat_org, pic_dyna, pic_stat, pic_dyna_enhance, pic_dyna_noised], '23')
show_Pic([pic_dyna_enhance_tailor, pic_stat_enhance_tailor, pic_dyna_enhance_tailor_reshape, pic_stat_enhance_tailor_reshape], '22')
