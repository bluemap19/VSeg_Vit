...
if __name__ == "__main__":
    ...
    for i in range(len(file_list)//2):
        ...
        # print(outputs.shape)
        outputs = cv2.resize(outputs, (pic_dyna.shape[0], pic_dyna.shape[1]))
        # show_Pic([pic_dyna[:, :], pic_stat[:, :], outputs*256], pic_order='13')
        # exit(0)
        path_t = r'D:\Data\target_stage3_small_p\train\1_p'
        cv2.imwrite(path_t+'/'+path_temp_stat.split('/')[-1].replace('stat', 'mask'), outputs.astype(int)*256)
